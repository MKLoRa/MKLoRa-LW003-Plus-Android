...
import com.moko.support.lw003plus.handler.MokoCharacteristicHandler;
...
public class LoRaLW003PlusMokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaLW003PlusMokoSupport INSTANCE;

    private Context mContext;

    private MokoBleConfig mBleConfig;

    private LoRaLW003PlusMokoSupport() {
        //no instance
    }

    public static LoRaLW003PlusMokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaLW003PlusMokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaLW003PlusMokoSupport();
                }
            }
        }
        return INSTANCE;
