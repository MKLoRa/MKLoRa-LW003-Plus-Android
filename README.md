# LW003Plus Android SDK Guide（English）
...
**`LW003Plus.LoRaLW003PlusMokoSupport`**
...
**`LW003Plus.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device
through `LoRaLW003PlusMokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW003PlusMokoSupport.We recommend putting it in Application.
...
in LoRaLW003PlusMokoSupport `AndroidManifest.xml`
...
```
LoRaLW003PlusMokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW003PlusMokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW003PlusMokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW003PlusMokoSupport` if you want to use
