...
import com.moko.lw003plus.databinding.Lw003PlusActivityPayloadBxpDeviceInfoContentBinding;
...
public class PayloadBXPDeviceInfoContentActivity extends BaseActivity {

    private Lw003PlusActivityPayloadBxpDeviceInfoContentBinding mBind;
    private boolean savedParamsError;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBind = Lw003PlusActivityPayloadBxpDeviceInfoContentBinding.inflate(getLayoutInflater());
                            ...
                            if (flag == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_BXP_DEVICE_INFO_CONTENT:
                                        ...
                                        if (savedParamsError) {
                                            ToastUtils.showToast(PayloadBXPDeviceInfoContentActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_BXP_DEVICE_INFO_CONTENT:
                                        if (length > 0) {
                                            ...
                                            mBind.cbDeviceProperty.setChecked((data & 0x80) == 0x80);
                                            mBind.cbSwitchStatus.setChecked((data & 0x0100) == 0x0100);
                                            mBind.cbFirmwareVersion.setChecked((data & 0x0200) == 0x0200);
                                            mBind.cbDeviceName.setChecked((data & 0x0400) == 0x0400);
    ...
    private void saveParams() {
        ...
        int flag = (mBind.cbMac.isChecked() ? 0x01 : 0x00)
                ...
                | (mBind.cbDeviceProperty.isChecked() ? 0x80 : 0x00)
                | (mBind.cbSwitchStatus.isChecked() ? 0x0100 : 0x00)
                | (mBind.cbFirmwareVersion.isChecked() ? 0x0200 : 0x00)
                | (mBind.cbDeviceName.isChecked() ? 0x0400 : 0x00)
                | (mBind.cbRawDataAdv.isChecked() ? 0x0800 : 0x00)
                | (mBind.cbRawDataResp.isChecked() ? 0x1000 : 0x00);
        orderTasks.add(OrderTaskAssembler.setPayloadBXPDeviceInfoContent(flag));
        LoRaLW003PlusMokoSupport.getInstance().sendOrder(orderTasks.toArray(new OrderTask[]{}));
    }
