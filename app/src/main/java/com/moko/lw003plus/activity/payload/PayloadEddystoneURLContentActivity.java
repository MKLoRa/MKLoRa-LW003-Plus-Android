...
import com.moko.lw003plus.databinding.Lw003PlusActivityPayloadEddystoneUrlContentBinding;
...
public class PayloadEddystoneURLContentActivity extends BaseActivity {

    private Lw003PlusActivityPayloadEddystoneUrlContentBinding mBind;
    private boolean savedParamsError;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBind = Lw003PlusActivityPayloadEddystoneUrlContentBinding.inflate(getLayoutInflater());
                            ...
                            if (flag == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_EDDYSTONE_URL_CONTENT:
                                        ...
                                        if (savedParamsError) {
                                            ToastUtils.showToast(PayloadEddystoneURLContentActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_EDDYSTONE_URL_CONTENT:
                                        if (length > 0) {
                                            ...
                                            mBind.cbUrl.setChecked((data & 0x10) == 0x10);
                                            mBind.cbRawDataAdv.setChecked((data & 0x20) == 0x20);
                                            mBind.cbRawDataResp.setChecked((data & 0x40) == 0x40);
                                        }
                                        break;
                                }
                            }
    ...
    private void saveParams() {
        ...
        int flag = (mBind.cbMac.isChecked() ? 0x01 : 0x00)
                ...
                | (mBind.cbUrl.isChecked() ? 0x10 : 0x00)
                | (mBind.cbRawDataAdv.isChecked() ? 0x20 : 0x00)
                | (mBind.cbRawDataResp.isChecked() ? 0x40 : 0x00);
        orderTasks.add(OrderTaskAssembler.setPayloadEddystoneURLContent(flag));
        LoRaLW003PlusMokoSupport.getInstance().sendOrder(orderTasks.toArray(new OrderTask[]{}));
    }
