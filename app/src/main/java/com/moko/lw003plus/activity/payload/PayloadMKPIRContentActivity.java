...
import com.moko.lw003plus.databinding.Lw003PlusActivityPayloadMkPirContentBinding;
...
public class PayloadMKPIRContentActivity extends BaseActivity {

    private Lw003PlusActivityPayloadMkPirContentBinding mBind;
    private boolean savedParamsError;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBind = Lw003PlusActivityPayloadMkPirContentBinding.inflate(getLayoutInflater());
                            ...
                            if (flag == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_BXP_PIR_CONTENT:
                                        ...
                                        if (savedParamsError) {
                                            ToastUtils.showToast(PayloadMKPIRContentActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_BXP_PIR_CONTENT:
                                        if (length > 0) {
                                            ...
                                            mBind.cbDelayRespStatus.setChecked((data & 0x08) == 0x08);
                                            mBind.cbDoorStatus.setChecked((data & 0x10) == 0x10);
                                            mBind.cbSensorSensitivity.setChecked((data & 0x20) == 0x20);
                                            mBind.cbSensorDetectionStatus.setChecked((data & 0x40) == 0x40);
                                            mBind.cbBatteryVoltage.setChecked((data & 0x80) == 0x80);
                                            mBind.cbMajor.setChecked((data & 0x0100) == 0x0100);
                                            mBind.cbMinor.setChecked((data & 0x0200) == 0x0200);
                                            mBind.cbRssi1m.setChecked((data & 0x0400) == 0x0400);
    ...
    private void saveParams() {
        ...
        int flag = (mBind.cbMac.isChecked() ? 0x01 : 0x00)
                ...
                | (mBind.cbDelayRespStatus.isChecked() ? 0x08 : 0x00)
                | (mBind.cbDoorStatus.isChecked() ? 0x10 : 0x00)
                | (mBind.cbSensorSensitivity.isChecked() ? 0x20 : 0x00)
                | (mBind.cbSensorDetectionStatus.isChecked() ? 0x40 : 0x00)
                | (mBind.cbBatteryVoltage.isChecked() ? 0x80 : 0x00)
                | (mBind.cbMajor.isChecked() ? 0x0100 : 0x00)
                | (mBind.cbMinor.isChecked() ? 0x0200 : 0x00)
                | (mBind.cbRssi1m.isChecked() ? 0x0400 : 0x00)
        ...
        orderTasks.add(OrderTaskAssembler.setPayloadBXPPIRContent(flag));
        LoRaLW003PlusMokoSupport.getInstance().sendOrder(orderTasks.toArray(new OrderTask[]{}));
    }
