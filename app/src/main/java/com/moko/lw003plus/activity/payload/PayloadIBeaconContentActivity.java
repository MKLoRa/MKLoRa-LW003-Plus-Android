...
import com.moko.lw003plus.databinding.Lw003PlusActivityPayloadIbeaconContentBinding;
...
public class PayloadIBeaconContentActivity extends BaseActivity {

    private Lw003PlusActivityPayloadIbeaconContentBinding mBind;
    private boolean savedParamsError;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mBind = Lw003PlusActivityPayloadIbeaconContentBinding.inflate(getLayoutInflater());
                            ...
                            if (flag == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_IBEACON_CONTENT:
                                        ...
                                        if (savedParamsError) {
                                            ToastUtils.showToast(PayloadIBeaconContentActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_PAYLOAD_IBEACON_CONTENT:
                                        if (length > 0) {
                                            ...
                                            mBind.cbRawDataAdv.setChecked((data & 0x80) == 0x80);
                                            mBind.cbRawDataResp.setChecked((data & 0x0100) == 0x0100);
                                        }
                                        break;
                                }
                            }
    ...
    private void saveParams() {
        ...
        int flag = (mBind.cbMac.isChecked() ? 0x01 : 0x00)
                ...
                | (mBind.cbRawDataAdv.isChecked() ? 0x80 : 0x00)
                | (mBind.cbRawDataResp.isChecked() ? 0x0100 : 0x00);
        orderTasks.add(OrderTaskAssembler.setPayloadIBeaconContent(flag));
        LoRaLW003PlusMokoSupport.getInstance().sendOrder(orderTasks.toArray(new OrderTask[]{}));
    }
