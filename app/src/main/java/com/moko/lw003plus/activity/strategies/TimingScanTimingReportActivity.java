...
import com.moko.lw003plus.databinding.Lw003PlusActivityTimingScanTimingReportBinding;
...
public class TimingScanTimingReportActivity extends BaseActivity implements BaseQuickAdapter.OnItemChildClickListener {
    ...
    private Lw003PlusActivityTimingScanTimingReportBinding mBind;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBind = Lw003PlusActivityTimingScanTimingReportBinding.inflate(getLayoutInflater());
        ...
        ScanSwipeToDeleteCallback scanCallback = new ScanSwipeToDeleteCallback();
        ItemTouchHelper scanHelper = new ItemTouchHelper(scanCallback);
        scanHelper.attachToRecyclerView(mBind.rvScanTimePoint);

        ReportSwipeToDeleteCallback reportCallback = new ReportSwipeToDeleteCallback();
        ItemTouchHelper reportHelper = new ItemTouchHelper(reportCallback);
        reportHelper.attachToRecyclerView(mBind.rvReportTimePoint);
...
//    };
    ...
    public class ScanSwipeToDeleteCallback extends ItemTouchHelper.Callback {
        ...
        public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
            ...
            mScanTimePoints.remove(position);
            int size = mScanTimePoints.size();
            for (int i = 1; i <= size; i++) {
                TimePoint point = mScanTimePoints.get(i - 1);
                point.name = String.format("Time Point %d", i);
    ...
    public class ReportSwipeToDeleteCallback extends ItemTouchHelper.Callback {
        ...
        public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
            int position = viewHolder.getAdapterPosition();
            mReportTimePoints.remove(position);
                        ...
                        if (value.length >= 5) {
                            ...
                            if (flag == 0x00) {
                                ...
                                switch (configKeyEnum) {
                                    case KEY_TIMING_SCAN_TIMING_REPORT_PARAMS:
                                        if (length > 0) {
                                            int duration = MokoUtils.toInt(Arrays.copyOfRange(value, 5, 5 + length));
